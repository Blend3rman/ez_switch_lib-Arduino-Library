/*
   ...
   output pin using a switch_lib function, so that when
...
struct switch_control {     // struct member meanings:
  ...
  bool  sw_output_level;    // define the status level of the defined output pin on set up
} btl[num_switches] = {     // 'btl' = buttons, toggles & LEDs

  //.................switch data...............>  <output pin initial setting level>
  ...
  do {
    for (byte sw = 0; sw < num_switches; sw++) {
      if (my_switches.read_switch(btl[sw].sw_id) == switched) {
        ...
        // flipp the associated switch output pins for us so we
